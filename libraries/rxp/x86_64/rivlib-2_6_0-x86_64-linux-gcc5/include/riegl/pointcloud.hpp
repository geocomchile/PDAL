...
protected:
    ...
    //! callback for detected gaps, i.e. shot without echo return
    //! the information is available from the time, beam_origin
    //! and beam_direction member variables
    //!\param none
    virtual void on_gap();
    ...
    virtual void on_check_parameters_before_measurement_starting(); //!< INTERNAL ONLY
    // overridden
    ...
    void on_device_geometry_7N(const device_geometry_7N<iterator_type>& arg); //!< INTERNAL ONLY
    ...
    void on_echo_gap_data(const echo_gap_data<iterator_type>& arg); //!< INTERNAL ONLY
    ...
    void on_mta_settings_3(const mta_settings_3<iterator_type>& arg); //!< INTERNAL ONLY
    ...
    void on_dyntrig_pre(const dyntrig_pre<iterator_type>& arg); //!< INTERNAL ONLY
    void on_dyntrig_post(const dyntrig_post<iterator_type>& arg); //!< INTERNAL ONLY
    ...
    void on_crc32_header(const crc32_header<iterator_type> &arg); //!< INTERNAL ONLY
    void on_crc32_check(const crc32_check<iterator_type> &arg); //!< INTERNAL ONLY
    void on_id(const package_id& id, const basic_package<iterator_type>& pkg); //!< INTERNAL ONLY
    virtual void on_crc32_error(uint16_t stream, uint32_t sequence_begin, uint32_t sequence_end); //!< INTERNAL ONLY
...
private:
    ...
    uint32_t frame_angle_raw; //!< INTERNAL ONLY

    unsigned num_segments; //!< INTERNAL ONLY
    //unsigned segment; //!< INTERNAL ONLY
...
private:
    unsigned segment_raw; //! INTERNAL ONLY
    ...
    bool is_rising_edge;//!< INTERNAL ONLY
    ...
    detail::clock_compensator gap_det_clock; //!< INTERNAL_ONLY
...
protected:
    ...
    bool is_dyntrig_2d; //!< INTERNAL ONLY
