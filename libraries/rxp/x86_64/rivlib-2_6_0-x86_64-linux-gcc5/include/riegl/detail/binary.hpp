...
struct utf8seq
{
    ...
    typedef base_type<unsigned, 8, little> utf8_bt;
    typedef field<char, utf8_bt, 0, It> field_type;

    utf8seq(It begin, It end, std::size_t& size, bool dirty = false)
        : data(begin)
        , max_size_(((end-begin)*BYTE_digits-offset)/8)
        , size_(size) // init reference
    {
      if (!((end-begin)*BYTE_digits >= signed(offset)+8))
        throw(std::range_error("Buffer too small for utf8seq."));
      clean();
      if (dirty)  { // need to initialize size_ from given buffer
        for (size_ = 0; size_ < max_size_; ++size_) {
            if ('\0' == (*this)[size_])
                break;
        }
      }
    ...
    {
        clean();
        for ( ; *arg && size_<max_size_; ++size_,++arg)
            (*this)[size_] = (const unsigned char)*arg;
        for (std::size_t n = size_ ; n<max_size_; ++n) (*this)[n] = '\0';
        return arg;
    }
...
//    inline It end(std::size_t index) const
//    {
//        return data+1+((offset+(index+1)*8-1)/BYTE_digits);
//    }
...
//        else
//            return data+1+((offset-1)/BYTE_digits);
        return data+1+((offset+(size_)*8-1)/BYTE_digits);
    }
...
protected:
...
#ifdef _MSC_VER
    #if (_MSC_VER > 1700)
        utf8seq(const utf8seq&) = delete;
        utf8seq& operator=(const utf8seq&) = delete;
    #endif
#else
    utf8seq(const utf8seq&) = delete;
    utf8seq& operator=(const utf8seq&) = delete;
    ...
    inline field_type operator[](std::size_t index) const
    {
        std::size_t index_offset = (offset+index*8)/BYTE_digits;
        unsigned begin_bit = unsigned((offset+index*8)%BYTE_digits);
        return field_type(data+index_offset, begin_bit);
    }
